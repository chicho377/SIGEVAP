...
namespace SIGEVAP.Views
{
    public partial class FrmMenu : Form
    {
        ...
        // ── Separadores visuales del dashboard ───────────────
        private Label _lblAccesosInicio;
        private Label _lblResumenInicio;
        ...
        private void ConfigurarContenidoDinamico()
        {
            ...
            ConfigurarSeccionesInicio();
            ConfigurarCardsDashboard();
            ConfigurarStatsDashboard();
        }

        private void ConfigurarSeccionesInicio()
        {
            _lblAccesosInicio = CrearTituloSeccionInicio("ACCESOS A MÓDULOS");
            _lblResumenInicio = CrearTituloSeccionInicio("RESUMEN GENERAL");

            pnlInicio.SuspendLayout();
            pnlInicio.Controls.Clear();
            pnlInicio.Controls.Add(flpStats);
            pnlInicio.Controls.Add(_lblResumenInicio);
            pnlInicio.Controls.Add(flpCards);
            pnlInicio.Controls.Add(_lblAccesosInicio);
            pnlInicio.Controls.Add(pnlWelcome);
            pnlInicio.ResumeLayout(true);
        }

        private Label CrearTituloSeccionInicio(string texto)
        {
            return new Label
            {
                AutoSize = false,
        ...
        private void ConfigurarStatsDashboard()
        {
            ...
            for (int i = 0; i < stats.Length; i++)
            {
                ...
                etiquetas[i].Font = new Font("Segoe UI", 10F);
                etiquetas[i].ForeColor = Color.FromArgb(105, 105, 105);
                etiquetas[i].Location = new Point(10, 82);
                etiquetas[i].Size = new Size(260, 30);
                etiquetas[i].Text = textos[i];
                etiquetas[i].TextAlign = ContentAlignment.MiddleCenter;
